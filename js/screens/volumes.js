...
(function (w) {
  ...
  w.Screens.volumes = {
    render: function () {
      ...
      for (i = 0; i <= 6; i++) {
        var bb = (i / 6) * bMax;
        xt += '<text x="' + vx(bb).toFixed(1) + '" y="186" fill="' + UI.color('--text-3') + '" font-size="10" text-anchor="middle">$' + bb.toFixed(2) + '</text>';
      }
        ...
        '<div class="card"><div class="card-h"><div class="card-t">Available volume</div>' +
          ...
          '<div class="card-b">' +
            ...
            '<div class="plot"><svg width="100%" height="200" viewBox="0 0 1104 200" fill="none" font-family="IBM Plex Mono, monospace" aria-label="Available volume by bid">' +
              ...
              '<line x1="' + vx(bid).toFixed(1) + '" y1="16" x2="' + vx(bid).toFixed(1) + '" y2="164" stroke="' + UI.color('--accent') + '" stroke-width="1" stroke-dasharray="3 3"/>' +
